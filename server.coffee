...
#
  ...
  allPlugins =  ->
    allPluginsLoc = path.join(argv.root, 'default-data', 'pages', 'all-plugins')
    allPlugins = {"title":"All Plugins", "story":[]};
    loga defargs(argv)
    glob "wiki-plugin-*", {cwd: argv.packageDir}, (e, files) ->
      if e then loga e
      files = files.map (file) -> file.slice(12)
      for file in files
        loga file
        filespec = argv.packageDir + path.sep + 'wiki-plugin-' + file + path.sep + 'pages' + path.sep + 'about-' + file.replace('wiki-','') + '-plugin'
        loga filespec
        try
          json = fs.readFileSync(filespec, 'utf-8')
          parsed = JSON.parse(json).story
          headline =  {  "type": "html", "id": random.random_id(), "text": '<h2>' + file + '</h2><hr>' }
          allPlugins.story.push(headline)
          allPlugins.story.push(parsed[0])
          example =  {  "type": "html", "id": random.random_id(), "text": '<h2>example of ' + file + '</h2><hr>' }
          allPlugins.story.push(example)
          allPlugins.story.push(parsed[1])
          loga allPlugins.story.length
        catch e
          loga e
          break
      loga 'writing ' + allPluginsLoc
      fs.writeFile allPluginsLoc, JSON.stringify(allPlugins), (e) ->
        if e then loga "all-plugins" + e
